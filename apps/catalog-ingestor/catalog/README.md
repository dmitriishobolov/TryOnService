...
- `providers` - реализации источников каталога: отдельная папка на магазин, `custom` для экспериментов, `tsum` для parser-а ЦУМ.
...
Если нужно быстро наполнить каталог вручную или проверить формат без настоящего парсинга сайта, используйте provider [custom](providers/custom/README.md) и `CATALOG_INGESTOR_CUSTOM_SOURCE_FILE`. Если пишем уникальный parser магазина, создаем отдельную provider-папку, как `providers/tsum`.
