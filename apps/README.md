...
- [coordinator](coordinator/README.md) - центральный сервис, который хранит jobs, регистрирует worker'ы/service clients и выдает клиентам worker assignment.
...
Код coordinator не должен напрямую зависеть от внутренних реализаций worker и не должен быть каналом передачи клиентского результата. Client отвечает за прямую отправку job назначенному worker'у, но не содержит бизнес-логику обработки примерки. Все общие форматы данных выносите в `shared`, чтобы контракты между сервисами оставались явными.
