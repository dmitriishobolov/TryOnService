...
3. Сценарий `Идеальный образ` просит фото в полный рост, загружает его в storage-node и создает OpenAI job с `payload.model.task=wardrobe-recommendation`.
4. Если фото не подходит для полного роста, worker возвращает JSON-отказ, а бот просит прислать другое изображение.
...
6. После выбора образа бот создает второй OpenAI job с web search: worker ищет реальные товарные карточки, пропускает неподходящие фото и не добавляет дубли категорий внутри одного образа.
...
- `Идеальный образ` переводит чат в состояние ожидания фото в полный рост, затем показывает до 3 вариантов образа и запускает поиск товаров по выбранному варианту.
...
- Фото в сценарии идеального образа создает OpenAI job со строгим JSON-ответом: `ok=false` для неподходящего фото или `ok=true` с массивом `outfits`.
- По выбранному образу создается второй OpenAI job с `options.webSearch`, чтобы найти товарные карточки в интернете.
- Для одного образа бот дополнительно отбраковывает дубли категорий и повторяющиеся ссылки: если уже выбран худи, второй худи в подборку не попадет.
