...
- `WILDBERRIES_API_KEY`, `WILDBERRIES_API_BASE_URL`, `WILDBERRIES_CARDS_LIST_PATH`, `WILDBERRIES_MAX_SCAN_CARDS`, `WILDBERRIES_LOCALE`, `WILDBERRIES_WITH_PHOTO`, `WILDBERRIES_PRODUCT_URL_TEMPLATE` - настройки Wildberries Content API. Adapter ищет среди карточек продавца, доступных этому token.
