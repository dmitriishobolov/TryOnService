...
- Telegram client подбирает свободный callback-порт, регистрируется в coordinator, по `/start` предлагает кнопку `Разбор внешности`, умеет отменять сценарий ожидания фото, получает assignment или `queued`-ответ, polling-ом дожидается свободного worker'а, отправляет job worker'у напрямую и выводит пользователю ответ worker'а; длинные ответы режутся на несколько сообщений и Markdown отображается форматированно; фото с подписью `/request openai:gpt-5.6-luna` также отправляется на OpenAI/ChatGPT vision adapter с выбранной моделью из запроса;
