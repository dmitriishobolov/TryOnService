...
OpenAI не принимает `web_search` вместе с `reasoningEffort=minimal`. Adapter автоматически поднимает такой запрос до `low`, а клиентские сценарии, которые используют web search, должны сразу задавать `low`.
