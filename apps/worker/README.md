...
7. Worker проверяет purpose/signature dispatch token, `workerId`, `jobId` и pending assignment, запускает runner и вызывает нужные adapters из `models`.
8. Worker отправляет progress/final status в coordinator по `x-worker-service-key` и клиентский результат напрямую в callback клиента с `x-client-callback-token`.
