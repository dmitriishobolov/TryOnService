...
- Prompt-ы для поиска и проверки используют compact JSON, `imageDetail=low`; web search идет с `reasoningEffort=medium`, чтобы настойчивее искать товары по синонимам, а vision-проверка и clean-card generation остаются на `low`. Validation-ответ короткий: `acceptedCandidates` с `canGenerateCleanCard`, чтобы снизить расход токенов.
