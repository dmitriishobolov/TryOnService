...
7. Worker проверяет purpose/signature dispatch token, `workerId`, `jobId`, текущий signing `keyVersion`, одноразовый `tokenId` и pending assignment, скачивает входные файлы по `StorageObjectRef`, запускает runner и вызывает adapter из `models`, выбранный клиентом в `payload.model.provider`; конкретная модель provider-а берется из `payload.model.providerModel`.
