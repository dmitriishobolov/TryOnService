...
function buildIdealOutfitPlanPrompt(
  ...
  return [
    ...
    "Не возвращай accepted=false только потому, что каталог не закрывает часть пожелания, обувь, аксессуары или конкретный тип вещи. Если фото подходит, accepted=true, а недоступные части пожелания адаптируй или пропусти.",
    "Если пожелание перечисляет больше 3 вещей, выбери 2-3 самые важные категории одежды, которые реально есть в catalogHints для targetGender и фильтров. Аксессуары и обувь выбирай только если такая category есть в catalogHints и g[targetGender] или g.unisex больше 0.",
    ...
    "Если подходит, верни accepted=true, targetGender и options из 3 вариантов. У каждого варианта должен быть свой styleName, summary, targetGender, userWish, sizePreference, pricePreference и categories. У каждой category тоже укажи gender, userWish, sizePreference и pricePreference.",
    ...
    "Если для пола и фильтров нет категории под желаемую идею, адаптируй идею к доступным категориям вместо того, чтобы планировать вещь, которой нет в каталоге. Это не причина отклонять фото.",
