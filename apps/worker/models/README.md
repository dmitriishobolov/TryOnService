...
Сейчас `inputImageUrls` используется Telegram-сценарием `Идеальный образ`: после поиска через API Ozon/Wildberries бот отправляет найденные product images на отдельную vision-проверку, где модель принимает кандидатов с `canGenerateCleanCard=true`, если из изображения можно надежно выделить один целевой товар. Фото товара на человеке или манекене допустимо, если предмет хорошо виден и его можно отделить от фона.
