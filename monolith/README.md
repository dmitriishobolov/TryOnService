...
9. После выбора стиль не отправляется в GPT для поиска по каталогу: монолит локально ищет кандидатов в `items.json`.
10. Для каждой категории берется shortlist товаров из локального каталога.
11. Перед TryOn бот делает компактную GPT-проверку изображений shortlist-а: модель смотрит фото вещей и отбраковывает чужие категории, белье вместо шорт/майки и явно неподходящие карточки.
12. Локальный скоринг выбирает лучший товар только из прошедших visual review кандидатов, с учетом категории, пола, цвета, размера, бюджета, тегов, описания и наличия `imageFile`.
13. TryOn-слой получает фото пользователя и локальные изображения выбранной одежды.
14. Бот отправляет результат примерки и карточки товаров с ценой, размерами, цветами и ссылкой.
...
- `MONOLITH_CATALOG_CANDIDATES_PER_CATEGORY` - сколько кандидатов брать на категорию для быстрой проверки доступности вариантов.
- `MONOLITH_CATALOG_VISUAL_REVIEW_CANDIDATES_PER_CATEGORY` - сколько кандидатов на категорию отправлять в GPT-проверку изображений перед TryOn.
