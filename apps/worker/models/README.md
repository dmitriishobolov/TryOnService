...
Сейчас реализован `mockTryOnModel.ts`: он имитирует обработку и возвращает сообщение `Ответ от сервера.` для проверки всей цепочки coordinator -> worker -> client callback.
