# Worker Models

`models` содержит реализации вызовов к AI API. Каждый provider или модель должны быть закрыты адаптером, чтобы runner работал с единым интерфейсом и не зависел от деталей конкретного внешнего API.

Сейчас реализован `mockTryOnModel.ts`: он имитирует обработку и возвращает сообщение `Ответ от сервера.` для проверки цепочки coordinator assignment -> direct client dispatch -> worker -> client callback.

## Что здесь размещать

- clients для внешних AI API;
- adapters под конкретные модели примерки;
- нормализацию запросов и ответов provider'а;
- обработку provider-specific ошибок;
- retry/timeout policy, если она относится именно к вызову модели.

## Что не размещать

- маршруты coordinator;
- бизнес-пайплайн обработки клиента;
- хранение job state;
- UI или клиентские интеграции.

## Рекомендуемый контракт adapter'а

Каждый adapter должен явно описывать:

- какие входные данные принимает;
- какие capabilities предоставляет;
- какой результат возвращает;
- какие ошибки являются retryable;
- какие лимиты есть у provider'а.

Runner должен выбирать adapter по типу job или capability, а не импортировать конкретный provider без необходимости.
