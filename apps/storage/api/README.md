...
- `POST /catalog/garments/categories` - internal список категорий `garment-item` с количеством записей; требует `x-storage-service-key` и вызывается coordinator-ом.
- `POST /catalog/garments/search` - internal поиск `garment-item` по категориям, тегам и тексту; требует `x-storage-service-key` и вызывается coordinator-ом.
...
Catalog entry не содержит бинарных данных. Он связывает cache key с уже загруженным объектом и optional metadata. Поддерживаемые cache kinds: `product-card-image`, `product-card-metadata`, `garment-item`. Для `garment-item` metadata должна содержать минимум `category`, желательно также `title`, `description`, `tags`, `price`, `currency`, `store`, `productUrl`. При lookup/search storage-node проверяет, что objectKey ещё существует, и удаляет устаревшие/битые записи из catalog index.
