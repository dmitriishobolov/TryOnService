...
Worker масштабируется горизонтально: поднимаем новый экземпляр, он сообщает coordinator о готовности, после чего coordinator может выдавать клиентам assignment на этот worker.
...
7. Worker проверяет purpose/signature dispatch token, `workerId`, `jobId`, текущий signing `keyVersion`, одноразовый `tokenId` и pending assignment.
...
- Новый бизнес-сценарий обработки добавляйте в [runner](runner/README.md), сохраняя HTTP-детали provider-ов внутри `models`.
