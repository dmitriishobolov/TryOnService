...
8. После выбора образа бот создает второй OpenAI job с web search: worker ищет несколько кандидатов товарных карточек для каждого элемента образа. Если на исходном фото обувь не видна, обувные категории не уходят в поиск.
9. Бот создает третий OpenAI vision job: worker получает найденные `imageUrl` как `payload.model.options.inputImageUrls` и проверяет, что на фото один товар без человека, манекена, других вещей, коллажей, текста и шумного фона.
