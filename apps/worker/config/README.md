...
- `WORKER_CAPABILITIES` - ручные дополнительные capabilities. Worker автоматически добавляет `try-on`, `try-on.mock`, `try-on.<provider>` для AI provider-ов с API keys.
...
- `OPENAI_API_KEY`, `OPENAI_API_BASE_URL`, `OPENAI_MODEL`, `OPENAI_IMAGE_DETAIL`, `OPENAI_TEXT_VERBOSITY`, `OPENAI_REASONING_EFFORT`, `OPENAI_REASONING_MODE`, `OPENAI_MAX_OUTPUT_TOKENS`, `OPENAI_STORE_RESPONSE`, `OPENAI_ORGANIZATION`, `OPENAI_PROJECT`, `OPENAI_SYSTEM_PROMPT`, `OPENAI_APPEARANCE_PROMPT` - настройки OpenAI/ChatGPT vision adapter для анализа внешности. `OPENAI_MODEL` используется только как fallback, если клиент не передал `payload.model.providerModel`.
